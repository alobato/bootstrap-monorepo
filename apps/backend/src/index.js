...
import { batchUsers, batchCustomers } from './loaders.js'

async function getContext(token, ip, host, userAgent) {
  try {
    if (!token) {
      return {
        token: null,
        ...
        loaders: {
          ...
          customer: new DataLoader((keys) => batchCustomers(keys))
        }
      }
    }
...
async function getSubscriptionContext(ctx, msg, args) {
  ...
  return { me: null } // Otherwise let our resolvers know we don't have a current user
}
